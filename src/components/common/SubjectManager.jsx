...
const SubjectManager = ({ subjects, setSubjects, branches }) => {
  ...
  const years = ['1', '2', '3', '4'];
  const classKey = `${selectedBranch}-${selectedYear}`;
  const classSubjects = subjects[classKey] || [];
  ...
  const handleSave = () => {
    ...
    setSubjects(prev => {
      ...
      if (editMode) {
        updated[classKey][editIndex] = currentSubject.trim();
      } else {
        if (updated[classKey].includes(currentSubject.trim())) {
          alert('Subject already exists for this class');
          return prev;
        }
        updated[classKey] = [...updated[classKey], currentSubject.trim()];
      }
  ...
  const handleDelete = (index) => {
    if (window.confirm('Are you sure you want to delete this subject?')) {
      setSubjects(prev => {
        const updated = { ...prev };
        updated[classKey] = updated[classKey].filter((_, i) => i !== index);
        return updated;
      });
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1">
            ...
            >
              {years.map(y => <option key={y} value={y}>{y}st Year</option>)}
            </select>
          </div>
        </div>
        ...
        <div className="p-6">
          {classSubjects.length > 0 ? (
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-slate-500">{classKey}</p>
                      </div>
                    </div>
            ...
            <div className="text-center py-12 text-slate-400">
              ...
              <p className="font-medium">No subjects added for {classKey}</p>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-6">
              ...
              <p className="text-xs text-slate-500 mt-2">
                For class: <span className="font-bold text-blue-600">{classKey}</span>
              </p>
            </div>
