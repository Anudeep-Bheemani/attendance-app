...
import { TrendingUp, TrendingDown, AlertTriangle, CheckCircle, XCircle, Search, Filter, ArrowUpDown, Calendar, Users, Activity, Bell } from 'lucide-react';
...
const AdminDashboard = ({ students, attendanceData, staffList }) => {
  ...
  const studentStats = useMemo(() => {
    return students.map(s => {
      const records = attendanceData.filter(r => r.studentId === s.id);
  ...
  const filteredStudents = useMemo(() => {
    ...
    if (selectedYear !== 'all') {
      filtered = filtered.filter(s => s.batch?.includes(selectedYear));
    }
  ...
  const sortedStudents = useMemo(() => {
    ...
    sorted.sort((a, b) => {
      if (sortConfig.key === 'percentage' || sortConfig.key === 'rollNo') {
        const aVal = sortConfig.key === 'percentage' ? a.percentage : a.rollNo;
        const bVal = sortConfig.key === 'percentage' ? b.percentage : b.rollNo;
        ...
        <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
          <table className="w-full text-base text-left">
            <thead className="bg-slate-50 text-slate-600 font-semibold border-b border-slate-200 sticky top-0">
              <tr>
                <th className="px-6 py-4 cursor-pointer hover:bg-slate-100" onClick={() => handleSort('rollNo')}>
                  <div className="flex items-center gap-2">
                    Roll No <ArrowUpDown size={16} />
                  </div>
                </th>
                <th className="px-6 py-4">Student Name</th>
                <th className="px-6 py-4 cursor-pointer hover:bg-slate-100" onClick={() => handleSort('percentage')}>
                  <div className="flex items-center gap-2">
                    Attendance % <ArrowUpDown size={16} />
                  </div>
                </th>
            ...
            <tbody className="divide-y divide-slate-100">
              {sortedStudents.map(student => {
                ...
                const year = yearMap[student.batch?.substring(0, 4)] || '1st Yr';
                return (
                <tr key={student.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4 font-mono text-slate-600">{student.rollNo}</td>
                  <td className="px-6 py-4">
                    <div className="font-medium text-slate-800">{student.name}</div>
                    <div className="text-sm text-slate-500">{student.branch} • {year}</div>
                  </td>
                  <td className="px-6 py-4">
